...
bool Handle_{{pkt.name}}(SharedPacketSession& session, Protocol::{{pkt.name}}&pkt);
{%- endfor %}
...
{%- for pkt in parser.recv_pkt %}
		GPacketHandler[PKT_{{pkt.name}}] = [](SharedPacketSession& session, BYTE* buffer, int32 len) { return HandlePacket < Protocol::{{pkt.name}} > (Handle_{{pkt.name}}, session, buffer, len); };
{%- endfor %}
	}
...
{%- for pkt in parser.send_pkt %}
	static SharedSendBuffer MakeSendBuffer(Protocol::{{pkt.name}}&pkt) { return MakeSendBuffer(pkt, PKT_{{pkt.name}}); }
{%- endfor %}
